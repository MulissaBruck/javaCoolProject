...
public class findNthPerfectNumber4 {
    ...
    public static int findNthPerfectNumber(int nth) {
        ...
        while (count < nth) {
            // Check if the sum of factors equals the number (perfect number check)
            while (sumOfFactors(number) != number && count != nth) {
                number++; // Move to the next number if it's not perfect
            }
            ...
            number++; // Check the next number after finding a perfect number
        }
